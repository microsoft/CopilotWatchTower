...
def test_directory_audits_persisted(repo: Repository) -> None:
    ...
    assert any(detail == "영향 사용자: 1명" for detail in outcome.details)
...
def test_directory_audits_filter_provisioning_noise(repo: Repository) -> None:
    fake = _FakeGraph(
        directory_audits=[
            {
                "id": "sync-1",
                ...
                "targetResources": [{"type": "User", "id": "u-x"}],
            },
            {
                "id": "app-1",
                "activityDateTime": "2025-01-15T09:31:00Z",
                "activityDisplayName": "Add service principal",
                "category": "ApplicationManagement",
                "initiatedBy": {"user": {"id": "u-admin", "userPrincipalName": "admin@x"}},
                "targetResources": [{"type": "ServicePrincipal", "id": "sp-1"}],
            },
        ]
    )
    outcome = aq.collect_entra_audits(repo, fake)  # type: ignore[arg-type]
    # The Azure AD Cloud Sync noise event is dropped at ingest.
    assert outcome.fetched == 1
    assert any(detail == "디렉터리 동기화 노이즈 제외: 1건" for detail in outcome.details)
    rows = repo.list_audit_events(source="entra_audit")
    assert {r.operation for r in rows} == {"Add service principal"}
    assert all(r.workload != "ProvisioningManagement" for r in rows)


def test_purge_audit_events_by_category(repo: Repository) -> None:
    from copilot_watchtower.db import AuditEventRow

    # Simulate a backlog collected before the ingest filter existed.
    repo.upsert_audit_events(
        [
            AuditEventRow(
                id="entra_audit:noise", source="entra_audit",
                event_time="2025-01-15T09:30:00Z", user_id=None, upn=None,
                operation="Import", workload="ProvisioningManagement", app="Core Directory",
                target_resources=None, client_ip=None, result="success",
                raw_json="{}", fetched_at="2025-01-15T09:30:00Z",
            ),
            AuditEventRow(
                id="entra_audit:keep", source="entra_audit",
                event_time="2025-01-15T09:31:00Z", user_id="u", upn="u@x",
                operation="Add service principal", workload="ApplicationManagement",
                app="Core Directory", target_resources=None, client_ip=None,
                result="success", raw_json="{}", fetched_at="2025-01-15T09:31:00Z",
            ),
        ]
    )
    removed = repo.purge_audit_events_by_category("entra_audit", aq.NOISE_AUDIT_CATEGORIES)
    assert removed == 1
    rows = repo.list_audit_events(source="entra_audit")
    assert {r.workload for r in rows} == {"ApplicationManagement"}
...
def test_sign_ins_persisted(repo: Repository) -> None:
    ...
    assert outcome.fetched == 1
    assert any(detail == "상위 앱: Microsoft 365 Copilot 1건" for detail in outcome.details)
...
def test_entra_batch_count_survives_flush(repo: Repository) -> None:
    ...
    assert any(detail == "영향 사용자: 3명" for detail in outcome.details)
...
def test_purview_submit_then_drain(repo: Repository) -> None:
    ...
    assert outcome.fetched == 1
    assert any(detail == "쿼리 ID: qid-1" for detail in outcome.details)
    assert any(detail == "상위 작업: CopilotInteraction 1건" for detail in outcome.details)
...
def test_purview_pending_query_resumes_next_cycle(repo: Repository) -> None:
    ...
    assert any(detail == "쿼리 상태: running" for detail in outcome.details)
    state = repo.get_audit_collection_state("purview")
    assert state is not None
