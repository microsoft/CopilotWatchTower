...
def _iso(day: int = 21) -> str:
    return datetime(2026, 5, day, 9, 0, tzinfo=UTC).isoformat().replace("+00:00", "Z")
...
def test_wipe_collected_data_clears_all_but_keeps_settings(tmp_path: Path) -> None:
    repo = _make_repo(tmp_path / "src" / "store.db")
    repo.set_text_setting("tenant_id", "secret-tenant")

    counts = repo.wipe_collected_data()

    # Collected data is gone, credentials/settings survive.
    assert repo.list_interactions(source_type=None, limit=10) == []
    assert repo.get_text_setting("tenant_id") == "secret-tenant"
    assert counts["interactions"] == 2
    assert counts["users"] == 1
    # Newer data tables are covered so a wipe is total.
    for table in ("power_platform_consumption", "flow_runs", "agent_definitions", "credit_alerts"):
