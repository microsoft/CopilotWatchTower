...
def test_wipe_profile_data_clears_collected_data(
    monkeypatch,
    qtbot,
    controller: OperationsController,
    repo: Repository,
    ...
    del qtbot
    from copilot_watchtower.db import InteractionRow, UserRow
    ...
    repo.upsert_interactions(
        [
            InteractionRow(
                ...
                created_at="2026-05-21T09:00:00Z", interaction_type="userPrompt",
                app="BizChat", body_text="hi", body_content_type="text",
                attachments_json=None, raw_json="{}", fetched_at="2026-05-21T09:00:00Z",
            )
        ]
    )
    repo.set_text_setting("tenant_id", "keep-me")

    # Run the maintenance task synchronously to assert it actually wipes.
    monkeypatch.setattr(
        controller,
        "_run_maintenance",
        lambda name, task: {"ok": True, "result": task(lambda *_a: None)},
    )

    result = controller.wipe_profile_data()

    assert result["ok"] is True
    assert result["result"]["deleted"] >= 2
    assert repo.list_interactions(source_type=None, limit=10) == []
    assert repo.get_text_setting("tenant_id") == "keep-me"


def test_remove_profile_skips_entra_delete_for_externally_managed_app(
    monkeypatch,
    qtbot,
    tmp_path: Path,
    repo: Repository,
    ...
    target_repo.set_text_setting("app_externally_managed", "1")
    del target_repo

    def _fail_provider(*_a: object, **_k: object) -> object:
        raise AssertionError("delegated token provider must not run for BYOA app")

    def _fail_registrar(*_a: object, **_k: object) -> object:
        raise AssertionError("AppRegistrar must not run for externally-managed app")

    monkeypatch.setattr(
        "copilot_watchtower.webshell.actions.DelegatedDeviceCodeTokenProvider", _fail_provider
    )
    monkeypatch.setattr("copilot_watchtower.webshell.actions.AppRegistrar", _fail_registrar)
    ...
    assert result["ok"] is True
    assert result["app_delete_skipped"] == "externally_managed"
