...
def _iso(hour: int = 12) -> str:
    return datetime(2026, 5, 1, hour, tzinfo=UTC).strftime("%Y-%m-%dT%H:%M:%SZ")
...
def test_parse_export_bytes_rejects_html_signin_page() -> None:
    # A non-zip payload that is actually a Microsoft sign-in page (returned when
    # the export session is not authenticated) must NOT be parsed as a bogus
    # conversation item. The page opens with a copyright comment before the
    # doctype, which earlier bypassed the HTML guard and was ingested as a thread.
    login_html = (
        b"<!-- Copyright (C) Microsoft Corporation. All rights reserved. -->\r\n"
        b"<!DOCTYPE html>\r\n"
        b'<html dir="ltr" class="" lang="en"><head>'
        b"<title>Sign in to your account</title></head>"
        b"<body><form></form></body></html>"
    )
    assert ediscovery_export.parse_export_bytes(login_html) == []


def test_looks_like_html_page_detects_comment_prefixed_login() -> None:
    from copilot_watchtower.services.ediscovery_browser_download import _looks_like_html
    from copilot_watchtower.services.ediscovery_export import _looks_like_html_page

    login_html = (
        b"<!-- Copyright (C) Microsoft Corporation. All rights reserved. -->\r\n"
        b"<!DOCTYPE html><html><head><title>Sign in to your account</title></head></html>"
    )
    assert _looks_like_html_page(login_html) is True
    assert _looks_like_html(login_html) is True
    # Real export payloads (ZIP / JSON / .eml) must not be misclassified.
    assert _looks_like_html_page(b"PK\x03\x04rest-of-zip-bytes") is False
    assert _looks_like_html_page(b'{"items": []}') is False
    assert _looks_like_html_page(b"Subject: hello\r\n\r\nplain body") is False


def test_current_page_is_sign_in_distinguishes_login_from_portal() -> None:
    from types import SimpleNamespace

    from copilot_watchtower.services.ediscovery_browser_download import current_page_is_sign_in

    # Still parked on a Microsoft sign-in domain → login did not complete.
    assert current_page_is_sign_in(
        SimpleNamespace(url="https://login.microsoftonline.com/common/oauth2/v2.0/authorize?x=1")
    ) is True
    assert current_page_is_sign_in(SimpleNamespace(url="https://login.live.com/login.srf")) is True
    # Reached the actual portal → signed in (any missing token is a permission issue).
    assert current_page_is_sign_in(
        SimpleNamespace(url="https://admin.powerplatform.microsoft.com/resources/capacity")
    ) is False
    assert current_page_is_sign_in(
        SimpleNamespace(url="https://make.powerautomate.com/environments")
    ) is False
    ...
    assert current_page_is_sign_in(_Detached()) is False
...
def test_parse_msg_uses_teams_itemdata_stream(monkeypatch) -> None:
    ...
    class FakeMsg:
        ...
        date = datetime(2026, 5, 1, 0, 2, 23, tzinfo=UTC)
