...
def test_thread_id_is_source_specific() -> None:
    # The same conversation (same user + Teams session) collected via two
    # sources must NOT collide on thread id, otherwise one source's upsert
    # overwrites the other's row — which made API conversations vanish from the
    # API view after an eDiscovery collection for the same user.
    turns = [
        _turn(1, session="19:abc@thread.v2", text="hello"),
        _turn(2, session="19:abc@thread.v2", type_="aiResponse", minute=1),
    ]
    api = te.compute_threads(turns, source_type="api")
    edi = te.compute_threads(turns, source_type="ediscovery")
    assert len(api) == 1 and len(edi) == 1
    assert api[0].id != edi[0].id
    # ...but the id stays stable within a source (so re-runs upsert, not duplicate).
    assert te.compute_threads(turns, source_type="api")[0].id == api[0].id
