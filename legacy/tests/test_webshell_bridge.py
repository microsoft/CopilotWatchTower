...
def test_bridge_conversation_detail_renders_adaptive_card_answer(qtbot, tmp_path: Path) -> None:
    """A Copilot answer delivered as an Adaptive Card attachment has only an
    ``<attachment id=...>`` placeholder in its body. The detail view must
    surface the card's text (the real answer), not the placeholder.
    """
    del qtbot
    db = tmp_path / "card.db"
    initialize(db)
    ...
    card_content = json.dumps(
        {
            "type": "AdaptiveCard",
            "body": [{"type": "TextBlock", "text": "추천 일정: 두오모 방문 후 갤러리아", "wrap": True}],
        },
        ensure_ascii=False,
    )
    ...
    repo.upsert_interactions(
        [
            InteractionRow(
                id="i1", user_id="u-1", session_id="sess", request_id=None,
                created_at="2026-05-01T09:00:00Z", interaction_type="userPrompt",
                app="IPM.SkypeTeams.Message.Copilot.WebChat", body_text="일정 짜줘",
                body_content_type="text", attachments_json=None,
                raw_json="{}", fetched_at="2026-05-01T09:00:00Z", thread_id=None,
            ),
            InteractionRow(
                id="i2", user_id="u-1", session_id="sess", request_id=None,
                created_at="2026-05-01T09:01:00Z", interaction_type="aiResponse",
                app="IPM.SkypeTeams.Message.Copilot.WebChat",
                body_text='<attachment id="att-1"></attachment>',
                body_content_type="html", attachments_json=attachments_json,
                raw_json="{}", fetched_at="2026-05-01T09:01:00Z", thread_id=None,
            ),
        ]
    )
    ...
    turns = [
        TurnInput(
            id=i.id, user_id=i.user_id, session_id=i.session_id, request_id=i.request_id,
            created_at=i.created_at, interaction_type=i.interaction_type, app=i.app,
            body_text=i.body_text, grounding_text=None,
        )
        for i in interactions
    ]
    threads = compute_threads(turns)
    ...
    summaries = json.loads(bridge.conversations_list("{}"))
    detail = json.loads(bridge.conversations_detail(summaries[0]["id"]))
    ai_turn = next(turn for turn in detail["turns"] if turn["interaction_type"] == "aiResponse")
    assert "두오모 방문 후 갤러리아" in ai_turn["body_text"]
    assert "<attachment" not in ai_turn["body_text"]
...
def test_bridge_conversations_api_falls_back_to_raw_identity_name(qtbot, tmp_path: Path) -> None:
    ...
    repo.upsert_threads(threads)
    repo.assign_threads_to_interactions(
        [(iid, t.id) for t in threads for iid in t.interaction_ids]
    )
    bridge = Bridge(BridgeContext(repo=repo))
...
def test_bridge_agents_list_marks_state_and_threshold(qtbot, tmp_path: Path) -> None:
    ...
    initialize(db)
    repo = Repository(db)
    # Dates relative to "now" so the stale/active threshold check does not
    # depend on the wall-clock date the suite runs on.
    from datetime import datetime, timedelta

    _now = datetime.now(UTC)
    captured_at = _now.strftime("%Y-%m-%dT%H:%M:%SZ")
    recent_event = (_now - timedelta(days=5)).strftime("%Y-%m-%dT%H:%M:%SZ")
    ...
    repo.upsert_audit_events(
        [
            AuditEventRow(
                id="a1", source="purview", event_time=recent_event,
