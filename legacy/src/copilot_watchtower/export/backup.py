        ...
        with _connect(source_db_path) as conn:
            ...
            try:
                for idx, table in enumerate(INCLUDED_TABLES, start=1):
                    if progress:
                        progress(translate("backup.progressBackup", table=table), idx, total)
...
def read_backup_manifest(bundle_path: Path) -> dict:
    ...
    with zipfile.ZipFile(bundle_path, "r") as zf, zf.open(MANIFEST_NAME) as fh:
...
def restore_backup_bundle(
    ...
    *,
        ...
        with tempfile.TemporaryDirectory(prefix="cwt-restore-") as tmp:
            ...
            with _connect(target.db_path) as conn:
                ...
                try:
                    for idx, table in enumerate(INCLUDED_TABLES, start=1):
                        if progress:
                            progress(translate("backup.progressRestore", table=table), idx, total)
    ...
    if progress:
        progress(translate("backup.recomputeThreads"), total, total)
