...
def export_thread_html(
    ...
    for idx, t in enumerate(threads):
        ...
        out.append(f"<h1 class='thread'>{html.escape(t.title or translate('export.untitled'))}</h1>")
        meta = (
            f"{translate('export.user')}: {html.escape(t.display_name or t.upn or t.user_id)} · "
            f"{translate('export.app')}: {html.escape(t.app or '-')} · "
            f"{translate('export.period')}: {html.escape(format_kst(t.started_at))} ~ {html.escape(format_kst(t.ended_at))} · "
            f"{translate('export.turns', turns=t.turn_count, prompts=t.prompt_count, responses=t.response_count)}"
        )
