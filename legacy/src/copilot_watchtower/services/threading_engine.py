...
class TurnPair:
    ...
    prompt: TurnInput | None
    response: TurnInput | None
...
def compute_threads(turns: Sequence[TurnInput], *, source_type: str = "api") -> list[ThreadGroup]:
    ...
    ``source_type`` is mixed into each thread ID so the same underlying
    conversation collected via different sources (e.g. the Graph API and an
    eDiscovery export share the same Teams session id) does not produce a
    colliding ID — which would otherwise let one source's upsert overwrite the
    other's thread row.
    """
    ...
    for user_id, user_turns in by_user.items():
        out.extend(_compute_user_threads(user_id, user_turns, source_type))
...
def _compute_user_threads(
    user_id: str, turns: Sequence[TurnInput], source_type: str = "api"
) -> list[ThreadGroup]:
    ...
    return [_finalise_thread(user_id, cluster, source_type) for cluster in clusters]
...
def _finalise_thread(
    user_id: str, cluster: list[_SessionSummary], source_type: str = "api"
) -> ThreadGroup:
    ...
    for s in cluster:
        ...
        for t in s.turns:
            ...
            if (t.interaction_type or "").lower() == "userprompt" and title is None and t.body_text:
    ...
    thread_id = _make_thread_id(user_id, session_ids or [str(t) for t in interaction_ids], source_type)
...
def _make_thread_id(user_id: str, parts: list[str], source_type: str = "api") -> str:
...
def _minutes_between(end_iso: str, start_iso: str) -> float | None:
