...
# Directory-audit categories that are operational noise for a Copilot
# governance tool: Azure AD Cloud Sync (directory synchronization) emits large
# volumes of ``ProvisioningManagement`` events that bury the security- and
# agent-relevant signal and are unrelated to Copilot. They are dropped at
# ingest, and any backlog is purged once (see app._purge_entra_audit_noise_once).
NOISE_AUDIT_CATEGORIES = frozenset({"ProvisioningManagement"})
...
def _counter_detail(label: str, counts: Counter[str], *, limit: int = 5) -> str | None:
    if not counts:
        return None
    summary = ", ".join(
        translate("audit.countItem", name=name, count=count)
...
def _purview_details(
    ...
    *,
    ...
    if query_id:
        details.append(translate("audit.queryId", query_id=query_id))
    if status:
        details.append(translate("audit.queryStatus", status=status))
    details.append(
        translate("audit.operationFilter", operations=", ".join(PURVIEW_DEFAULT_OPERATIONS))
    )
    return details
    ...
    *,
    ...
    try:
        ...
        if ge.status == 403 and _is_purview_permission_error(ge.detail):
            ...
            state.pending_window_end = None
            state.last_error = (
                translate("audit.purview403")
                + "\n"
                + translate("audit.purviewPermissionHint")
            )
            state.last_error_at = _iso_now()
            repo.update_audit_collection_state(state)
            return CollectionOutcome(source="purview", fetched=0, error=str(ge))
...
def _drain_purview_query(
    ...
    if user_keys:
        details.append(translate("audit.affectedUsers", count=len(user_keys)))
    op_detail = _counter_detail(
        translate("audit.topOperations"), Counter(row.operation or "(unknown)" for row in rows)
    )
    if op_detail:
    ...
    *,
    ...
    try:
        ...
        for raw in stream:
            ...
            if row is None:
                continue
            # Skip directory-sync noise (Azure AD Cloud Sync etc.) so the
            # security/agent-relevant audit signal is not buried.
            if source == "entra_audit" and (row.workload or "") in NOISE_AUDIT_CATEGORIES:
                filtered += 1
                continue
            rows.append(row)
            total += 1
        ...
        if filtered:
            details.append(translate("audit.filteredNoise", count=filtered))
        if user_keys:
            details.append(translate("audit.affectedUsers", count=len(user_keys)))
        counter_label = (
            translate("audit.topOperations")
            if source == "entra_audit"
            else translate("audit.topApps")
        )
