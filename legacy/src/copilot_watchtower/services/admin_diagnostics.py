...
def collect_copilot_admin_diagnostics(
    ...
    *,
    ...
    """
    probes = [
        ("limited_mode", translate("diag.label.limitedMode"), "/copilot/admin/settings/limitedMode", graph.get_copilot_admin_limited_mode, _summarize_limited_mode),
        ("policy_settings", translate("diag.label.policySettings"), "/copilot/admin/policySettings", graph.list_copilot_admin_policy_settings, lambda p: _summarize_collection(p, translate("diag.unit.policy"))),
        (
            "catalog_packages",
            translate("diag.label.catalog"),
            "/copilot/admin/catalog/packages",
            (catalog_graph or graph).list_copilot_admin_catalog_packages,
            lambda p: _summarize_collection(p, translate("diag.unit.package")),
        ),
        ("agent_registrations", translate("diag.label.agentRegistrations"), "/copilot/agentRegistrations", graph.list_copilot_agent_registrations, lambda p: _summarize_collection(p, translate("diag.unit.registration"))),
    ]
    ...
    if on_step is not None:
        with contextlib.suppress(Exception):
            on_step("persist", translate("diag.persistStep"), None)
    ...
    *,
    ...
    try:
        ...
        return CopilotAdminDiagnosticRow(
            ...
            status="error",
            status_code=401,
            summary=translate("diag.delegatedExpired"),
            payload_json=None,
            error=str(exc),
...
def _summarize_limited_mode(payload: Any) -> str:
    if not isinstance(payload, dict):
        return translate("diag.responseReceived")
    enabled = payload.get("isEnabledForGroup")
    group_id = payload.get("groupId")
    if enabled is True:
        return translate("diag.limitedModeOn", group=group_id or translate("diag.groupUnknown"))
    if enabled is False:
        return translate("diag.limitedModeOff")
    return translate("diag.limitedModeResponse")


def _summarize_collection(payload: Any, label: str) -> str:
    if isinstance(payload, list):
        return translate("diag.collectionCount", label=label, count=f"{len(payload):,}")
    return translate("diag.collectionResponse", label=label)
...
def _error_summary(status: str, detail: str | None = None) -> str:
    if status == "forbidden" and detail and _is_agent365_license_error(detail):
        return translate("diag.agent365")
    return {
        "forbidden": translate("diag.forbidden"),
        "not_found": translate("diag.notFound"),
    }.get(status, translate("diag.callFailed"))
...
def _unexpected_error_summary(exc: Exception) -> str:
    ...
    if not message:
        return translate("diag.unexpectedError")
    return translate("diag.unexpectedErrorDetail", message=message)
