...
<body><div class="box"><h1>✕ %%HEADING%%</h1>
<p>%%ERRORLABEL%% <code>%%ERROR%%</code></p>
    ...
    def start(self) -> None:
        ...
        class Handler(BaseHTTPRequestHandler):
            ...
            def do_GET(self) -> None:  # noqa: N802
                ...
                if error:
                    body = (
                        _FAILURE_PAGE.replace("%%LANG%%", translate("consent.htmlLang"))
                        .replace("%%HEADING%%", translate("consent.failureHeading"))
                        .replace("%%ERRORLABEL%%", translate("consent.errorLabel"))
                        .replace("%%ERROR%%", error)
                    ...
                    body = (
                        _SUCCESS_PAGE.replace("%%LANG%%", translate("consent.htmlLang"))
                        .replace("%%HEADING%%", translate("consent.successHeading"))
                        .replace("%%BODY%%", translate("consent.successBody"))
                    ).encode("utf-8")
                    self.send_response(200)
                    self.send_header("Content-Type", "text/html; charset=utf-8")
                    self.send_header("Content-Length", str(len(body)))
