...
class GraphClient:
    ...
    def granted_app_role_ids(self, client_id: str) -> set[str]:
        """Return the Microsoft Graph app-role IDs granted to ``client_id``.

        Resolves the service principal by application (client) ID and reads its
        ``appRoleAssignments`` (the application permissions actually consented
        to the app). Each ``appRoleId`` is a globally unique GUID matching the
        role definition on the resource, so the returned set can be compared
        directly against the required Graph role GUIDs.

        Requires ``Application.Read.All`` (or ``Directory.Read.All``) on the
        calling app. When that permission is missing Graph returns 403 and this
        raises :class:`GraphError`; the caller treats that as "cannot verify"
        rather than "no roles granted".
        """
        r = self._request(
            "GET",
            f"{MS_GRAPH_BASE_V1}/servicePrincipals(appId='{client_id}')",
            params={"$select": "id"},
        )
        sp_object_id = str(r.json().get("id") or "")
        if not sp_object_id:
            return set()
        role_ids: set[str] = set()
        url = f"{MS_GRAPH_BASE_V1}/servicePrincipals/{sp_object_id}/appRoleAssignments"
        for raw in self._paginate(url, {"$select": "appRoleId", "$top": 999}):
            role_id = raw.get("appRoleId")
            if role_id:
    ...
    def list_audit_query_records(self, query_id: str) -> Iterator[dict[str, Any]]:
        ...
        yield from self._paginate(url, {"$top": 200})
        ...
        * ``403`` — token is valid but the signed-in user lacks the
        ...
        try:
            ...
            if exc.status == 403:
                raise GraphError(
                    403,
                    translate("graph.ediscoveryForbidden"),
                ) from exc
            if exc.status == 401:
                raise GraphError(
                    401,
                    translate("graph.ediscoveryUnauthorized"),
                ) from exc
            raise
...
def _needs_aad_bearer(url: str) -> bool:
    ...
    return not host.endswith(".blob.core.windows.net")
