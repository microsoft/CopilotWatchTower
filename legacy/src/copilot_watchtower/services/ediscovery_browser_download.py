...
def download_with_playwright(
    ...
    *,
    ...
    if not username or not password:
        raise BrowserDownloadError(translate("consumption.noCredentials"))
        ...
        try:
            try:
                ...
                if isinstance(exc, PlaywrightTimeoutError):
                    raise BrowserDownloadError(
                        translate("ediscovery.browserDownloadTimeout")
                    ) from exc
                raise BrowserDownloadError(
                    translate("ediscovery.browserDownloadFailed", error=exc)
                ) from exc
        finally:
            browser.close()
...
def _download_with_browser_cookies(
    ...
    *,
    ...
    with (
        httpx.Client(timeout=httpx.Timeout(600.0, connect=30.0), follow_redirects=True, cookies=cookie_dict) as client,
...
def current_page_is_sign_in(page: Any) -> bool:
    """Best-effort: True if the headless browser is still parked on a Microsoft
    sign-in page, meaning the login did not complete (wrong credentials or an
    MFA prompt). Callers use this to distinguish "couldn't sign in" from
    "signed in but the account lacks the Power Platform access to mint a token".
    """
    try:
        url = page.url or ""
    ...
    return host.startswith("login.") or host == "msft.sts.microsoft.com"
...
def _assert_not_html(path: Path) -> None:
    ...
    if _looks_like_html(header):
        with contextlib.suppress(OSError):
            path.unlink()
        raise BrowserDownloadError(translate("ediscovery.browserHtmlResponse"))


def _looks_like_html(data: bytes) -> bool:
    head = data[:1024].lstrip().lower()
    # Sign-in / error pages often open with the Microsoft copyright comment
    # before the doctype, so skip any leading HTML comment(s) first.
