...
def fetch_latest_release(
    *,
    ...
    try:
        ...
        raise VersionCheckError(translate("versionCheck.networkError", error=exc)) from exc
    finally:
        if owns_client:
            http.close()

    if resp.status_code == 404:
        raise VersionCheckError(translate("versionCheck.noReleases"))
    if not resp.is_success:
        raise VersionCheckError(
            translate("versionCheck.fetchFailed", status=resp.status_code)
        )
    try:
        payload = resp.json()
    except ValueError as exc:
        raise VersionCheckError(translate("versionCheck.parseFailed")) from exc
