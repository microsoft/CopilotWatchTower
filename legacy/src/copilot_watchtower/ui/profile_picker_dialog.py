...
class ProfilePickerDialog(QDialog):
    ...
    def __init__(self, registry: ProfileRegistry, parent: QWidget | None = None) -> None:
        ...
        self.setWindowTitle(translate("profilePicker.title"))
        self.resize(560, 420)

        header = QLabel(translate("profilePicker.header"))
        ...
        # Side buttons
        self.add_btn = QPushButton(translate("profilePicker.addBtn"))
        self.add_btn.clicked.connect(self._on_add)
        self.rename_btn = QPushButton(translate("profilePicker.renameBtn"))
        self.rename_btn.clicked.connect(self._on_rename)
        self.delete_btn = QPushButton(translate("profilePicker.deleteBtn"))
        ...
        ok.setText(translate("profilePicker.openBtn"))
        cancel = self.buttons.button(QDialogButtonBox.Cancel)
        cancel.setText(translate("dlg.cancel"))
    ...
    def _format_label(profile: Profile) -> str:
        ...
        if tenant_label:
            bits.append(translate("profilePicker.tenantLabel", tenant=tenant_label))
        elif profile.tenant_id:
            bits.append(
                translate("profilePicker.tenantLabel", tenant=f"{profile.tenant_id[:8]}…")
            )
        if not profile.bootstrap_complete:
            bits.append(translate("profilePicker.incomplete"))
        return " — ".join(bits)
    ...
    def _on_add(self) -> None:
        name, ok = QInputDialog.getText(
            self,
            translate("profilePicker.newProfileTitle"),
            translate("profilePicker.displayNameLabel"),
            text=translate("profilePicker.newTenantDefault"),
        )
        if not ok:
            return
        name = name.strip()
    ...
    def _on_rename(self) -> None:
        ...
        name, ok = QInputDialog.getText(
            self,
            translate("profilePicker.renameTitle"),
            translate("profilePicker.displayNameLabel"),
            text=profile.name,
        )
        if not ok:
    ...
    def _on_delete(self) -> None:
        ...
        ans = QMessageBox.warning(
            self,
            translate("profilePicker.deleteTitle"),
            translate("profilePicker.deleteConfirm", name=profile.name),
        ...
        try:
            ...
            QMessageBox.critical(
                self,
                translate("profilePicker.deleteTitle"),
                translate("profilePicker.dbOpenFailed", error=e),
            )
            return
    ...
    def _on_accept(self, *_args) -> None:
        ...
        if profile is None and not self.add_requested:
            QMessageBox.information(
                self,
                translate("profilePicker.profileTitle"),
                translate("profilePicker.selectPrompt"),
            )
            return
