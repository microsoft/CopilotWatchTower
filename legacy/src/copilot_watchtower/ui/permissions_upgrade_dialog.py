...
class PermissionsUpgradeDialog(QDialog):
    ...
    def __init__(self, repo: Repository, parent: QWidget | None = None) -> None:
        ...
        self.repo = repo
        self.setWindowTitle(translate("permsUpgrade.title"))
        self.resize(560, 460)
        ...
        # --- header
        header = QLabel(translate("permsUpgrade.header"))
        header.setWordWrap(True)

        # --- device code area
        self.code_label = QLabel(translate("permsUpgrade.issuingCode"))
        ...
        self.status = QLabel(translate("permsUpgrade.preparing"))
        self.status.setWordWrap(True)

        # --- consent button (shown after patch succeeds)
        self.consent_btn = QPushButton(translate("permsUpgrade.consentBtn"))
        ...
        if not self._tenant_id or not self._app_id:
            ...
            self.status.setText(translate("permsUpgrade.noTenantApp"))
            return
    ...
    def _on_prompt(self, prompt: DeviceCodePrompt) -> None:
        ...
        self.status.setText(translate("permsUpgrade.signingIn"))
    ...
    def _on_signed_in(self, result: DeviceCodeResult) -> None:
        ...
        self.status.setText(
            translate("permsUpgrade.signedIn", upn=result.upn or "(unknown)")
        )
        self._start_patch()
    ...
    def _on_patched(self, app_object_id: str) -> None:
        ...
        self.bar.setVisible(False)
        self.status.setText(translate("permsUpgrade.patched"))
    ...
    def _open_admin_consent(self) -> None:
        ...
        if self._consent_server is None:
            self.status.setText(translate("permsUpgrade.callbackNotReady"))
            return
        ...
        self.consent_btn.setEnabled(False)
        self.bar.setVisible(True)
        self.status.setText(translate("permsUpgrade.waitingConsent"))
    ...
    def _on_consent_done(self, ok: bool, error: object) -> None:
        ...
        self.bar.setVisible(False)
        if ok:
            self.status.setText(translate("permsUpgrade.consentDone"))
            mark_purview_permissions_ready(self.repo)
            audit("settings.permission_upgrade_consent_granted")
        else:
            self.consent_btn.setEnabled(True)
            self.status.setText(
                translate("permsUpgrade.consentFailed", error=error or "unknown")
            )
            audit(
