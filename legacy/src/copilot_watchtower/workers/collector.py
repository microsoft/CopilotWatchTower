...
from ..services.capabilities import COPILOT_SERVICE_PLAN_IDS
...
# Microsoft 365 Copilot service plan IDs live in services.capabilities (the
# home for license-detection knowledge) and are imported above. Matching any
# of them against a SKU's service plans flags a Copilot-licensed user.
...
class CollectorWorker(QObject):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            with contextlib.suppress(Exception):
                self.error.emit(translate("worker.collectionAborted", error=e))
        finally:
    ...
    def _run_inner(self) -> None:
        ...
        self.log_line.emit(translate("worker.started", run_id=run_id, trigger=self.trigger))
        self.progress.emit(translate("worker.preparing"), 1)
        ...
        try:
            ...
            self.log_line.emit(translate("worker.targetUsers", count=len(users)))
            if not users:
                self.log_line.emit(translate("worker.noTargetUsers"))
            for index, user in enumerate(users, start=1):
                if self._should_stop:
                    self.log_line.emit(translate("worker.stoppedByUser"))
                    break
                pct = int(100 * index / max(1, len(users)))
                display = user.display_name or user.upn or user.id
                self.progress.emit(display, pct)
                self.log_line.emit(translate("worker.userQuerying", index=index, total=len(users), display=display))
                try:
    ...
    def _sync_users(self) -> set[str] | None:
        ...
        self.log_line.emit(translate("worker.userSyncStart", mode=mode))
        self.progress.emit(translate("worker.userSyncProgress", mode=mode), 2)
        try:
            in_scope_ids = self._resolve_scope_user_ids()
        except GraphError as ge:
            self.error.emit(translate("worker.userListFailed", error=ge))
            return None
        self.log_line.emit(translate("worker.inScopeTargets", count=f"{len(in_scope_ids):,}"))

        # Fetch full user attributes for everything that ended up in scope.
        self.log_line.emit(translate("worker.usersDetailQuerying"))
        self.progress.emit(translate("worker.usersQuerying"), 4)
        ...
        try:
            ...
            for gu in self.graph.list_users():
                ...
                count += 1
                if count % 200 == 0:
                    self.log_line.emit(translate("worker.usersReceived", count=f"{count:,}", in_scope=f"{len(graph_users):,}"))
        except GraphError as ge:
            self.error.emit(translate("worker.usersDetailFailed", error=ge))
            return None
        self.log_line.emit(translate("worker.usersQueryDone", total=f"{count:,}", in_scope=f"{len(graph_users):,}"))

        if mode in {"ALL_ACTIVE", "GROUP", "CUSTOM"}:
            self.log_line.emit(translate("worker.copilotLicenseQuerying"))
            self.progress.emit(translate("worker.copilotLicenseProgress"), 6)
            licensed_ids = set(self.graph.copilot_licensed_user_ids(self._copilot_sku_ids()))
            self.log_line.emit(translate("worker.licensedUsers", count=f"{len(licensed_ids):,}"))
        else:
        ...
        self.log_line.emit(translate("worker.userSyncDone", count=len(rows)))
        self.progress.emit(translate("worker.userSyncDoneProgress"), 8)
        return {r.id for r in rows}

    def _resolve_scope_user_ids(self) -> set[str]:
        mode = self.options.scope_mode
        if mode == "ALL_ACTIVE":
            self.log_line.emit(translate("worker.allActiveQuerying"))
        ...
        if mode == "LICENSED":
            ...
            self.log_line.emit(translate("worker.licensedUsersShort", count=f"{len(ids):,}"))
            return ids
        if mode == "GROUP":
            if not self.options.scope_group_id:
                raise RuntimeError("Group scope requires a group ID.")
            self.log_line.emit(translate("worker.groupMemberQuerying", group_id=self.options.scope_group_id))
            return set(self.graph.group_member_ids(self.options.scope_group_id))
        if mode == "CUSTOM":
            ids: set[str] = set()
            for upn in self.options.scope_upns:
                self.log_line.emit(translate("worker.upnResolving", upn=upn))
    ...
    def _collect_user(self, user: UserRow) -> int:
        ...
        if fetched_count > 0:
            self.log_line.emit(
                translate(
                    ...
                    latest=format_kst(latest_returned_created),
                )
            )
        else:
            window = (
                translate("worker.sinceWindow", since=format_kst(since))
                if since
                else translate("worker.fullBackfill")
            )
            self.log_line.emit(
                translate("worker.userApiReturnedZero", display=display, window=window)
            )
        ...
        try:
            ...
            self.error.emit(translate("worker.threadComputeFailed", user=user.upn or user.id, error=e))
...
class AuditCollectorWorker(QObject):
    ...
    def run(self) -> None:
        ...
        try:
            ...
            errors += 1
            with contextlib.suppress(Exception):
                self.error.emit(translate("worker.audit.collectionAborted", label=label, error=e))
        finally:
    ...
    def _collect_audit_events(self) -> tuple[int, int]:
        ...
        for label, fn in (
            ...
            try:
                ...
                errors += 1
                self.error.emit(translate("worker.audit.collectionFailed", label=label, error=e))
                continue
            self.audit_progress.emit(outcome.source, outcome.fetched)
            total += outcome.fetched
            if outcome.error:
                errors += 1
                self.log_line.emit(translate("worker.audit.outcomeError", label=label, error=outcome.error))
            elif outcome.pending:
                self.log_line.emit(translate("worker.audit.outcomePending", label=label))
            else:
                self.log_line.emit(translate("worker.audit.outcomeFetched", label=label, count=outcome.fetched))
            for detail in outcome.details:
                self.log_line.emit(translate("worker.audit.outcomeDetail", label=label, detail=detail))
        return total, errors
    ...
    def _collect_admin_diagnostics(self) -> tuple[int, int]:
        ...
        def on_step(phase: str, label: str, detail: str | None) -> None:
            if phase == "started":
                state["started"] += 1
                self.log_line.emit(translate("worker.diag.stepStarted", label=label, current=state["started"], total=total_steps))
                ...
                state["finished"] += 1
                summary = detail or translate("worker.diag.done")
                self.log_line.emit(translate("worker.diag.stepFinished", label=label, summary=summary))
                percent = int((state["finished"] / total_steps) * 100)
                self.audit_progress.emit(label, percent)
            elif phase == "persist":
                self.log_line.emit(translate("worker.diag.persisting"))
                self.audit_progress.emit(translate("worker.diag.persistLabel"), 99)

        self.log_line.emit(translate("worker.diag.startTotal"))
        self.audit_progress.emit(translate("worker.diag.label"), 0)
        try:
            ...
            self.error.emit(translate("worker.diag.delegatedExpired"))
            return 0, 1
        except Exception as e:  # noqa: BLE001
            log.exception("Copilot admin diagnostics failed")
            self.error.emit(translate("worker.diag.failed", error=e))
            return 0, 1
        finally:
            if catalog_graph is not None:
                catalog_graph.close()
        self.log_line.emit(translate("worker.diag.itemsUpdated", count=rows))
        ...
        if agent_source_failed and not agent_source_ok and agent_count:
            ...
            self.log_line.emit(
                translate("worker.diag.agentListKept", count=f"{agent_count:,}")
            )
        else:
            self.log_line.emit(translate("worker.diag.agentListSaved", count=f"{agent_count:,}"))
        self.audit_progress.emit(translate("worker.diag.label"), 100)
        return rows, errors

    def _build_delegated_catalog_graph(self) -> GraphClient | None:
        tenant_id = self.repo.get_text_setting("tenant_id")
        if not tenant_id:
            self.log_line.emit(translate("worker.diag.noTenantSkip"))
            return None
    ...
    def _snapshot_usage_report(self) -> tuple[int, int]:
        ...
        for period in USAGE_REPORT_PERIODS:
            ...
            try:
                ...
                self.error.emit(translate("worker.usage.collectionFailed", period=period, error=e))
                errors += 1
        ...
        if total > 0:
            self.log_line.emit(translate("worker.usage.reportUpdated", total=total, detail=detail, latest=latest))
        else:
            self.log_line.emit(translate("worker.usage.reportNoRows", detail=detail or translate("worker.usage.noPeriod")))
        return total, errors


def _management_collection_label(data_kind: str) -> str:
    return {
        "audit": translate("worker.label.audit"),
        "usage": translate("worker.label.usage"),
        "diagnostics": translate("worker.label.diagnostics"),
