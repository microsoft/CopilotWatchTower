...
-- Per-agent credit analysis groups by ``product`` (the agent name in
-- MCSMessages:resource rows); index it so the agent delta/trend queries stay fast.
CREATE INDEX IF NOT EXISTS ix_pp_consumption_product
    ON power_platform_consumption(report_type, product, usage_date DESC);

-- v6 additions (agent credit governance):
--   * flow_runs          — Power Automate / Copilot Studio flow execution
--                          history from the Dataverse ``flowrun`` elastic table.
--                          Freshest available signal for autonomous / scheduled
--                          agents (no human, so no useful transcript). Counts
--                          are a leading indicator, NOT billed credits.
--   * agent_definitions  — per-agent static risk profile derived from the
--                          Dataverse ``bot`` / ``botcomponent`` definition
--                          (OBI ``data``). Predicts risky agents before runaway.
--   * credit_alert_rules — admin-tunable thresholds for the alert engine.
--   * credit_alerts      — fired alert history across all three signal tiers.

CREATE TABLE IF NOT EXISTS flow_runs (
    id               TEXT PRIMARY KEY,   -- flowrunid (GUID)
    environment_id   TEXT,
    environment_name TEXT,
    workflow_id      TEXT,               -- owning flow (workflowid / _workflow_value)
    workflow_name    TEXT,
    modern_flow_type INTEGER,            -- 0 PowerAutomate | 1 CopilotStudioFlow | 2 M365CopilotAgentFlow
    conversation_id  TEXT,               -- Copilot Studio conversation id (human-linked when present)
    bot_id           TEXT,               -- resolved agent GUID when derivable
    owner_id         TEXT,               -- _ownerid_value
    owner_name       TEXT,
    status           TEXT,               -- Succeeded | Failed | Running | Cancelled | ...
    trigger_type     TEXT,
    ...
    error_code       TEXT,
    error_message    TEXT,
    run_date         TEXT,               -- date(start_time|createdon) for daily grouping
    created_on       TEXT,               -- Dataverse createdon (freshness + incremental watermark)
    raw_json         TEXT,
    captured_at      TEXT NOT NULL
);

CREATE INDEX IF NOT EXISTS ix_flow_runs_created ON flow_runs(created_on DESC);
CREATE INDEX IF NOT EXISTS ix_flow_runs_run_date ON flow_runs(run_date DESC);
CREATE INDEX IF NOT EXISTS ix_flow_runs_workflow ON flow_runs(workflow_id);
CREATE INDEX IF NOT EXISTS ix_flow_runs_bot ON flow_runs(bot_id);
CREATE INDEX IF NOT EXISTS ix_flow_runs_status ON flow_runs(status);

CREATE TABLE IF NOT EXISTS agent_definitions (
    id                       TEXT PRIMARY KEY,  -- bot GUID (botid)
    environment_id           TEXT,
    environment_name         TEXT,
    bot_name                 TEXT,
    schema_name              TEXT,
    ...
    has_trigger              INTEGER NOT NULL DEFAULT 0,  -- autonomous / trigger present
    external_call_count      INTEGER NOT NULL DEFAULT 0,  -- connector + HTTP calls
    tool_count               INTEGER NOT NULL DEFAULT 0,  -- plugin / action / tool invocations
    loop_count               INTEGER NOT NULL DEFAULT 0,  -- loop / iteration constructs
    knowledge_count          INTEGER NOT NULL DEFAULT 0,  -- knowledge sources
    generative_orchestration INTEGER NOT NULL DEFAULT 0,  -- generative orchestration / Custom GPT
...
CREATE INDEX IF NOT EXISTS ix_agent_def_env ON agent_definitions(environment_id);
CREATE INDEX IF NOT EXISTS ix_agent_def_risk ON agent_definitions(risk_score DESC);

-- Admin-tunable thresholds for the multi-signal credit alert engine. One row
-- per rule key; the meaning of ``threshold``/``secondary`` is rule-specific
-- (documented in services/credit_alerts.py). Seeded with conservative
-- defaults below so the engine works out of the box and admins only tune.
CREATE TABLE IF NOT EXISTS credit_alert_rules (
    rule_key   TEXT PRIMARY KEY,
    enabled    INTEGER NOT NULL DEFAULT 1,
    threshold  REAL,
    secondary  REAL,
    severity   TEXT NOT NULL DEFAULT 'warn',   -- default severity: warn | danger
    updated_at TEXT
);

-- Fired credit alerts across all three signal tiers (billing / realtime /
-- predictive). Deduped per (rule, scope, day) so a re-evaluation refreshes an
-- existing alert instead of piling up duplicates.
CREATE TABLE IF NOT EXISTS credit_alerts (
    id               TEXT PRIMARY KEY,  -- sha1(rule_key|scope_id|usage_date)
    rule_key         TEXT NOT NULL,
    severity         TEXT NOT NULL,     -- warn | danger
    tier             TEXT NOT NULL,     -- billing | realtime | predictive
    scope_type       TEXT NOT NULL,     -- agent | user | flow | tenant
    scope_id         TEXT,
    scope_label      TEXT,
    environment_name TEXT,
    metric           REAL,              -- observed value that tripped the rule
    threshold        REAL,              -- threshold crossed
    baseline         REAL,              -- baseline/context where relevant
    usage_date       TEXT,              -- day/window the alert concerns
    detail_json      TEXT,
...
CREATE INDEX IF NOT EXISTS ix_credit_alerts_status ON credit_alerts(status, last_seen DESC);
CREATE INDEX IF NOT EXISTS ix_credit_alerts_rule ON credit_alerts(rule_key);

-- Default rule set (conservative). ``threshold``/``secondary`` meaning:
--   agent_daily_abs    threshold = messages/day per agent
--   user_daily_abs     threshold = messages/day per user
--   spike              threshold = delta/baseline ratio; secondary = min baseline floor
--   monthly_budget     threshold = projected messages/month (tenant); disabled until tuned
--   flow_spike         threshold = runs/baseline ratio; secondary = min baseline daily runs
--   flow_fail_loop     threshold = failure rate (0..1); secondary = min runs today
--   runaway_autonomous threshold = autonomous runs in window
--   high_risk_agent    threshold = static risk score (0..100)
INSERT OR IGNORE INTO credit_alert_rules(rule_key, enabled, threshold, secondary, severity) VALUES
    ('agent_daily_abs',    1, 500,     0,  'danger'),
    ('user_daily_abs',     1, 300,     0,  'warn'),
    ('spike',              1, 3,       50, 'warn'),
    ('monthly_budget',     0, 1000000, 0,  'warn'),
    ('flow_spike',         1, 3,       20, 'warn'),
    ('flow_fail_loop',     1, 0.5,     20, 'danger'),
    ('runaway_autonomous', 1, 1000,    0,  'danger'),
    ('high_risk_agent',    1, 75,      0,  'warn');
