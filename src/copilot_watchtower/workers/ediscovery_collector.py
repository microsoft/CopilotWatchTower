...
class EdiscoveryCollectorWorker(QObject):
    ...
    def _browser_download(self, url: str) -> bytes | None:
        ...
        if not user or password_blob is None:
            self.log_line.emit("• headless 브라우저 자동 로그인 계정이 저장되어 있지 않습니다.")
            return None
        password = str(unprotect(password_blob))
        self.log_line.emit("• headless 브라우저 자동 로그인으로 eDiscovery 패키지 다운로드를 시도합니다.")
        try:
    ...
    def run(self) -> None:
        ...
        try:
            ...
            self.log_line.emit(f"○ eDiscovery 수집 시작: {self.target_upn}")
            resume_stages = {"case", "searching", "exporting", "downloading", "parsing"}
            if (job.status or "").lower() in resume_stages:
                self.log_line.emit(f"↻ 중단된 작업을 '{job.status}' 단계부터 이어서 진행합니다.")
    ...
    def _run_import(self) -> None:
        ...
        try:
            ...
            if not path.is_file():
                raise EdiscoveryError(f"가져올 파일을 찾을 수 없습니다: {self.import_path}")
            job = self._load_or_create_job()
            user = self._ensure_target_user_offline(job)
            self.log_line.emit(f"○ eDiscovery 가져오기 시작: {path.name}")
            self.progress.emit("parsing", "다운로드한 내보내기 파일을 읽는 중...")
            data = path.read_bytes()
            self.progress.emit(
                "parsing", f"파일 로드 완료 ({_human_size(len(data))}) · 패키지 해석을 시작합니다"
            )
            ...
            job.updated_at = _now_iso()
            self.repo.upsert_ediscovery_job(job)
            self.progress.emit("done", f"가져오기 완료: {added}건 추가")
        except EdiscoveryError as e:
            log.warning("eDiscovery import failed: %s", e)
            errors = 1
            self.error.emit(f"eDiscovery 가져오기 실패: {e}")
            self.progress.emit("error", str(e))
        except Exception as e:  # noqa: BLE001 — absolute backstop
            log.exception("eDiscovery import aborted: %r", e)
            errors = 1
            try:
                self.error.emit(f"eDiscovery 가져오기 장애로 중단되었습니다: {e}")
            except Exception:
                pass
        finally:
            self.cycle_finished.emit(self.target_upn, added, errors)
    ...
    def _build_delegated_graph(self) -> GraphClient | None:
        ...
        if not tenant_id:
            self.error.emit("eDiscovery 수집: 테넌트 ID가 없어 delegated 로그인을 할 수 없습니다.")
            return None
    ...
    def _mark_job_auth_expired(self) -> None:
        ...
        try:
            job = self._load_or_create_job()
            job.status = "error"
            job.last_error = "위임 로그인 만료 — 설정 → 권한 재등록 필요"
