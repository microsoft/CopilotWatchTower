...
def render_thread_markdown(thread: ThreadRow, turns: list[InteractionRow]) -> str:
    ...
    lines: list[str] = []
    lines.append(f"# {thread.title or '(제목 없음)'}")
    lines.append("")
    lines.append(f"- 사용자: {thread.display_name or thread.upn or thread.user_id}")
    lines.append(f"- 앱: {thread.app or '-'}")
    lines.append(
        f"- 기간 (KST): {format_kst(thread.started_at)} ~ {format_kst(thread.ended_at)}"
    )
    lines.append(
        f"- 턴: {thread.turn_count} (질문 {thread.prompt_count} / 응답 {thread.response_count})"
    )
    lines.append("")
    pairs = pair_turns(
        [
    ...
    for idx, pair in enumerate(pairs, start=1):
        if pair.prompt is not None:
            lines.append(
                f"## Turn {idx} — 사용자 [{format_kst(pair.prompt.created_at)}]"
            )
            lines.append("")
        ...
        if pair.response is not None:
            lines.append(
                f"### Copilot 응답 [{format_kst(pair.response.created_at)}]"
            )
            lines.append("")
