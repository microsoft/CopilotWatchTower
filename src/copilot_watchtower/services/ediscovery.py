...
def _download_progress_text(
    ...
    *,
    ...
    if total > 0:
        ...
        parts = [f"다운로드 {_human_size(done)} / {_human_size(total)} ({pct}%)"]
    else:
        parts = [f"다운로드 {_human_size(done)}"]
    if rate_bps > 0:
        parts.append(f"{_human_size(int(rate_bps))}/s")
    if eta_seconds is not None and eta_seconds > 0:
        parts.append(f"남은 시간 {_human_duration(eta_seconds)}")
    return " · ".join(parts)
...
class EdiscoveryOrchestrator:
    ...
    def _poll_operation(self, job: EdiscoveryJob, operation_url: str, label: str) -> dict[str, Any]:
        ...
        labels = {"Estimate": "추정", "Export": "내보내기"}
        ...
        while True:
            ...
            self._progress(
                job.status,
                f"{ko_label} 진행 중 · 경과 {_human_duration(elapsed)} "
                f"(서버 상태={status or 'running'}, {attempt}회 확인)",
            )
            if status in _TERMINAL:
                ...
                self._progress(job.status, f"{ko_label} 완료 · 경과 {_human_duration(elapsed)}")
                return payload
        ...
        * nothing persisted             -> full (re)build from preflight
        ...
        if job.export_url:
            if _is_reports_download_url(job.export_url) and job.operation_url:
                self._progress(
                    "downloading",
                    "저장된 다운로드 URL이 Reports 메타 ZIP이라 operation metadata에서 Items 패키지를 다시 선택합니다",
                )
    ...
    def _finish_export(self, job: EdiscoveryJob, final: dict[str, Any]) -> list[InteractionRow]:
        ...
        try:
            top_keys = sorted(k for k in final.keys() if not k.startswith("@"))
    ...
    def _download_and_parse(self, job: EdiscoveryJob) -> list[InteractionRow]:
        ...
        self._save(job, "downloading")
        self._progress("downloading", "내보내기 패키지 다운로드를 시작합니다")
        ...
        try:
            ...
            if getattr(exc, "auth_redirect", False):
                ...
                if rows is not None:
                    return rows
                raise EdiscoveryError(
                    "내보내기는 완료됐지만 다운로드 프록시가 브라우저 대화형 "
                    "id_token 세션만 허용합니다. 자동 브라우저 다운로드가 설정되지 "
                    "않았거나 실패했습니다."
                ) from exc
            ...
            if _is_direct_download_proxy(job.export_url):
                ...
                if rows is not None:
                    return rows
                raise EdiscoveryError(
                    "내보내기는 완료됐지만 다운로드 링크가 브라우저 전용 프록시 "
                    "링크입니다(Direct Download Proxy). 앱의 백엔드 토큰 후보가 "
                    "모두 거부되었고 자동 브라우저 다운로드도 실패했습니다."
                ) from exc
            if (
                ...
                self._progress("exporting", "내보내기 링크가 만료되어 다시 내보냅니다")
                job.export_url = None
        ...
        self._progress(
            "parsing", f"다운로드 완료 ({_human_size(len(data))}) · 패키지 해석을 시작합니다"
        )
    ...
    def _browser_download_fallback(
        ...
        *,
        ...
        if not data:
            raise EdiscoveryError("headless 브라우저 자동 다운로드가 실패했습니다.")
        self._progress(
            "parsing", f"브라우저 다운로드 완료 ({_human_size(len(data))}) · 패키지 해석을 시작합니다"
        )
        return self._parse_package(job, data)
    ...
    def _parse_package(self, job: EdiscoveryJob, data: bytes) -> list[InteractionRow]:
        ...
        self._progress(
            "parsing", f"해석 완료 · {len(items)}개 항목에서 상호작용 {len(rows)}건을 추출했습니다"
        )
        return rows
