    ...
    try:
        ...
        try:
            ...
            raise DataverseError(
                resp.status_code,
                "BAP 환경 목록 응답이 JSON이 아닙니다. 토큰/권한을 확인하세요.",
            ) from exc
...
def _describe_content_shape(content: Any) -> str:
    ...
    if isinstance(data, (str, bytes)):
        ...
        try:
            ...
            return f"비 JSON 문자열(앞부분: {stripped[:60]!r})"
    if isinstance(data, dict):
        keys = ", ".join(list(data.keys())[:8]) or "(키 없음)"
        return f"dict(키: {keys})"
    if isinstance(data, list):
        return f"list(길이 {len(data)})"
    if data is None:
        return "없음(None)"
...
def parse_conversation_transcript(
    ...
    *,
    ...
    if human_user_id != "dataverse:unknown":
        ...
        result.participants.setdefault(human_user_id, "알 수 없는 사용자")
    return result
...
def repair_dataverse_attribution(repo: Any) -> int:
    ...
    log.info(
        "Dataverse 귀속 보정 완료: 상호작용 %d건, 사용자 %d명 갱신",
        updated,
        len(user_rows),
    )
    return updated
...
class DataverseClient:
    ...
    def _headers(self, url: str, *, page_size: int | None = None) -> dict[str, str]:
        headers = {
    ...
    def _get_json(
        self, url: str, *, params: dict[str, str] | None = None, page_size: int | None = None
    ) -> Any:
        resp = self._client.get(url, headers=self._headers(url, page_size=page_size), params=params)
        if not resp.is_success:
            raise DataverseError(resp.status_code, _safe_text(resp))
        try:
            return resp.json()
            ...
            raise DataverseError(
                resp.status_code,
                "예상치 못한 응답(비 JSON)입니다. Dataverse 토큰/권한을 확인하세요.",
            ) from exc
