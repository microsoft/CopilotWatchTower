...
def try_self_add_as_admin(
    ...
    *,
    ...
    _log(f"  ↳ {label}: 관리 센터에서 나를 시스템 관리자로 추가 시도 중…")
    try:
        page.goto(admin_url, timeout=45_000, wait_until="domcontentloaded")
    except Exception:  # noqa: BLE001 - best effort
        _log(f"  • {label}: 관리 센터 페이지를 열지 못했습니다(건너뜁니다).")
        return False
    try:
        page.wait_for_load_state("networkidle", timeout=20_000)
    except Exception:
    ...
    if not clicked:
        _log(
            f"  • {label}: '나 추가' 버튼을 찾지 못했습니다. 관리 센터 화면이 "
            f"바뀌었거나 권한이 없을 수 있습니다(건너뜁니다)."
        )
        return False
    ...
    _log(f"  ✓ {label}: 나를 시스템 관리자로 추가했습니다(권한 반영을 기다립니다).")
    return True
...
class CapturedDataverseTokens:
    ...
    def token_for(self, url: str) -> str:
        ...
        raise DataverseBrowserError(
            f"'{host}' 호스트에 대한 Dataverse 토큰을 찾지 못했습니다."
        )
...
def capture_dataverse_tokens(
    *,
    ...
    if not username or not password:
        raise DataverseBrowserError("headless 브라우저 로그인 계정/비밀번호가 없습니다.")
    ...
    with sync_playwright() as playwright:
        ...
        try:
            _log("• 메이커 포털 자동 로그인으로 Dataverse 접근을 시도합니다.")
            try:
            ...
            if enumerate_environments is not None and bap_token:
                ...
                if pending:
                    _log(
                        f"• 환경 {len(pending)}개의 토큰을 발급받는 중입니다…"
                    )
                for target in pending:
                    ...
                    if minted or target.org_host in tokens:
                        _log(f"  ✓ {target.label} 토큰을 발급받았습니다.")
                    else:
                        ...
                        if recovered or target.org_host in tokens:
                            _log(
                                f"  ✓ {target.label} 토큰을 발급받았습니다"
                                f"(관리자 추가 후)."
                            )
                        else:
                            _log(
                                f"  • {target.label} 토큰을 발급받지 못했습니다"
                                f"(이 환경은 건너뜁니다). 대상 호스트: {target.org_host}"
                            )

            if tokens:
                _log(
                    f"• Dataverse 토큰 {len(tokens)}개를 확보했습니다: "
                    + ", ".join(sorted(tokens))
                )
                return CapturedDataverseTokens(tokens)

            raise DataverseBrowserError(
                "Dataverse 토큰을 찾지 못했습니다. 저장된 계정이 하나 이상의 "
                "Power Platform 환경에 접근할 수 있는지 확인하세요."
            )
        finally:
            try:
                browser.close()
