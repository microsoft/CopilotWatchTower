  ...
  return (
    <div className="card livelog">
      <div className="card-head">
        <h2>
          {running ? <Loader2 size={14} className="spin" /> : <Terminal size={14} />} 실시간 로그
        </h2>
        <span className="hint">
          {running ? '진행 중' : lines.length ? '완료' : '대기'} · {lines.length.toLocaleString('ko-KR')}줄
          {pct != null ? ` · ${pct}%` : ''}
        </span>
        <div className="livelog-tools">
          <button className="btn-sm" onClick={copy} disabled={!lines.length} title="로그 복사">
            <Copy size={13} /> 복사
          </button>
          {onClear && (
            <button className="btn-sm danger" onClick={onClear} disabled={!lines.length || running} title="로그 지우기">
              <Eraser size={13} /> 지우기
            </button>
          )}
        </div>
      ...
      <div className="livelog-body mono" ref={bodyRef} onScroll={onScroll}>
        {shown.length === 0 ? (
          <div className="livelog-empty">수집을 시작하면 진행 로그가 실시간으로 표시됩니다.</div>
        ) : (
          shown.map((l, i) => (
